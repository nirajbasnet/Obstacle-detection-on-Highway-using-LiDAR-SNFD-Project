...
#include "quiz/cluster/EuclideanClustering.h"
...
template<typename PointT>
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>
ProcessPointClouds<PointT>::MyRansacSegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations,
                                                 float distanceThreshold) {
    // Time segmentation process
    auto startTime = std::chrono::steady_clock::now();
    pcl::PointIndices::Ptr inliers(new pcl::PointIndices());
    std::unordered_set<int> inliers_set = Segment3DPlane<PointT>(cloud, maxIterations, distanceThreshold);
    ...
    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    std::cout << "MyRansac3d plane segmentation took " << elapsedTime.count() << " milliseconds" << std::endl;
...
ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize,
    ...
    for (pcl::PointIndices individual_indices: cluster_indices) {
        ...
        for (const int idx : individual_indices.indices)
...
template<typename PointT>
std::vector<typename pcl::PointCloud<PointT>::Ptr>
ProcessPointClouds<PointT>::MyEuclideanClustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance,
    ...
    // Get the cluster IDs
    const std::vector<std::vector<int>> clusters_from_kdtree = EuclideanClustering(points_as_vec, tree,
                                                                                   clusterTolerance);
    delete tree;
    tree = nullptr;

    // Loop through each cluster and filter only those whose size is within the min-max range
    ...
    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    std::cout << "My Euclidean clustering took " << elapsedTime.count() << " milliseconds and found " << clusters.size()
              << " clusters" << std::endl;
